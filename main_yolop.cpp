...
Mat YOLO::detect(Mat & srcimg)
{
    ...
    for (size_t i = 0; i < indices.size(); ++i)
    {
        int idx = indices[i];
        Rect box = boxes[idx];
        rectangle(outimg, Point (box.x, box.y),Point(box.x + box.width, box.y + box.height),
                  Scalar(0, 0, 255), 2);
        //cv::rectangle(outimg,Point (box.x, box.y),Point(box.x + box.width, box.y + box.height),Scalar(0,255,0),2,8);
...
int main()
{
    YOLO yolo_model("/home/oliver-cui/data1/project/data/yolop/yolop.onnx", 0.4, 0.45, 0.5);
    string imgpath = "/home/oliver-cui/data1/project/data/yolop/images/0ace96c3-48481887.jpg";
    std::cout<<"beigin to read img!"<<std::endl;
    ...
    //waitKey(0);
    imwrite("/home/oliver-cui/data1/project/data/yolop/res_img.jpg",outimg);
    //destroyAllWindows();
    return 0;
}
